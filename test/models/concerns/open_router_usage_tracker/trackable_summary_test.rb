...
class TrackableTest < ActiveSupport::TestCase
  ...
  test "#usage_today returns the correct summary for today" do
    summary = OpenRouterUsageTracker::DailySummary.create!(user: @user, day: @today, total_tokens: 100, cost: 0.1, model: "yeah-yeah")
  ...
  test "#cost_exceeded? returns true if the daily cost limit is exceeded" do
    OpenRouterUsageTracker::DailySummary.create!(user: @user, day: @today, cost: 1.5, model: "yeah-yeah")
    assert @user.cost_exceeded?(limit: 1.0)
  end

  test "#cost_exceeded? returns false if the daily cost limit is not exceeded" do
    OpenRouterUsageTracker::DailySummary.create!(user: @user, day: @today, cost: 0.5, model: "yeah-yeah")
